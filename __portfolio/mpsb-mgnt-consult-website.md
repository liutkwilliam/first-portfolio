---
layout: portfoliosite
categories:
- ux ui web
title: MPSB Management Consulting Website
description: A website design and frontend development for MPSB Management Consulting.
organisation: MPSB Management Consulting
deliverables: Website design, frontend development and logo design
date: 2021-12-07 12:00:00 +1000
folder: mpsb-mgnt-consult-website
image: mpsb-portfolio.png
slug: mpsb-mgnt-consult-website
link:
feature:
not-show:
images:
    - image_path: mpsb-home.png
      title: Home Page
    - image_path: mpsb-service.png
      title: Services Page
    - image_path: mpsb-details.png
      title: Services Detail Page
---
Completed Website: [www.mpsb.com.au](http://www.mpsb.com.au)

I have the pleasure designing a logo and a website for a friend on his personal business MPSB Management Consulting ("MPSB").

MPSB is passionate about helping SMEs achieve next level growth in their business.

The website design is mainly kept simple and straightforward using the colour palette, images supplied by the client.

<!-- I have also produced a simple loop animation for MPSB slogan "Focus Plan Act Deliver Repeat". -->

##### Acknowledgement:
 
I acknowledge Milan for the opportunity to work together on the project.